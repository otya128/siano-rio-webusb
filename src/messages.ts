// Ported from linux/drivers/media/common/siano/smscoreapi.h
// Copyright (C) 2006-2008, Uri Shkolnik, Anatoly Greenblat (Siano Mobile Silicon). GPL-2.0-or-later.

/** Host <-> device message types (enum msg_types). */
export const Msg = {
...
/** smscore_translate_msg(): symbolic name for logging. */
export function messageName(type: number): string {
  return names.get(type) ?? `Unknown msg type ${type}`;
}

/** Message source/destination ids. */
export const HIF_TASK = 11;
export const SMS_HOST_LIB = 150;
export const DVBT_BDA_CONTROL_MSG_ID = 201;
/** Header flag: payload starts at response_alignment + ((flags >> 8) & 3) instead of right after the header. */
export const MSG_HDR_FLAG_SPLIT_MSG = 4;
export const SMS_MAX_PAYLOAD_SIZE = 240;
/** SMS_PROTOCOL_MAX_RAOUNDTRIP_MS */
export const SMS_PROTOCOL_MAX_ROUNDTRIP_MS = 10000;
...
export type DeviceModeValue = (typeof DeviceMode)[keyof typeof DeviceMode];
